...
Initially the controller and robot dynamics simulation were built into the same ODE, and while Matlab's ODE solver was able to solve it, it took a long time. Because of this the nonlinear controller was discretized, and used the ODE23 solver to solve many small steps, between these steps the nonlinear controller equation was numerically recalculated. Then the non-linear controller was tested by using $u = [1,0,0]^T$, $u = [0,1,0]^T$,$u = [0,0,1]^T$ as test cases. The results for this are shown in figures \ref{fig:ControlTestX} - \ref{fig:ControlTestZ}.
...
The linearization of the error was achieved by using the Linear Controller. This controller inputs the position and velocity errors, and outputs the direction of motion for the end-effector. This was verified to be working by using Cases 1-4 seen in figures \ref{fig:Case1}-\ref{fig:Case4}. The controller was able to calculate the proper direction to move the end effector using the error.
